...
class HbSendEmail(object):
    ...
    def send(self, email_title, email_content):
        ...
        msg['Subject'] = email_title.decode('utf-8').encode('gbk')           # email Subject
...
if __name__ == "__main__":
    ...
    email_tiltle = 'API'+'%s'%current_time       # as johnny test_2016-06-20_Monday ,it can choose only file when add time
    email_content = 'python发送邮件测试，包含附件'

    myemail = HbSendEmail(email_config_path,email_attachment_path)
    myemail.connect()
    myemail.login()
